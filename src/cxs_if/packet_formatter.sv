module packet_formatter #(
    ...
    assign async_fifo_w_data = current_flit_data[word_count*WORD_WIDTH +: WORD_WIDTH];
    ...
    if (!rst_n) begin
        ...
        if (current_state == S_CAPTURE) begin
            current_flit_data <= flit_fifo_rdata[CXS_DATA_WIDTH-1:0];
            cxs_ctrl          <= flit_fifo_rdata[FLIT_WIDTH-1 -: CXS_CNTL_WIDTH];
            word_count        <= '0;
        end
        else if (current_state == S_STREAM && !async_fifo_full) begin
